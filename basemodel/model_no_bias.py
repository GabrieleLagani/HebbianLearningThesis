...
class Net(nn.Module):
	...
	def __init__(self, input_shape=P.INPUT_SHAPE):
		...
		self.conv2 = nn.Conv2d(96, 128, 3, bias=False) # 96 input channels, 128 output channels, 3x3 convolutions
		self.bn2 = nn.BatchNorm2d(128, affine=False) # Batch Norm layer
		# Third convolutional layer
		self.conv3 = nn.Conv2d(128, 192, 3, bias=False)  # 128 input channels, 192 output channels, 3x3 convolutions
		self.bn3 = nn.BatchNorm2d(192, affine=False) # Batch Norm layer
		# Fourth convolutional layer
		self.conv4 = nn.Conv2d(192, 256, 3, bias=False)  # 192 input channels, 256 output channels, 3x3 convolutions
		self.bn4 = nn.BatchNorm2d(256, affine=False) # Batch Norm layer
		
		self.conv_output_size = utils.shape2size(utils.get_conv_output_shape(self))
		
		# FC Layers
		self.fc5 = nn.Linear(self.conv_output_size, 300, bias=False) # conv_output_size-dimensional input, 300-dimensional output
		self.bn5 = nn.BatchNorm1d(300, affine=False) # Batch Norm layer
		self.fc6 = nn.Linear(300, P.NUM_CLASSES, bias=False) # 300-dimensional input, 10-dimensional output (one per class)
